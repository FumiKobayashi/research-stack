...
_SESSION_ID="$$-$(date +%s)"
_TEL_START=$(date +%s)
~/.claude/skills/research-stack/bin/rstack-timeline-log '{"skill":"report","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
```bash
~/.claude/skills/research-stack/bin/rstack-learnings-log '{"skill":"report","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
~/.claude/skills/research-stack/bin/rstack-timeline-log '{"skill":"report","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Write `research/reports/<slug>.md` using a **finding-centric** layout. The
guiding principle: each finding's claim, supporting figure, supporting data,
and interpretation live in the **same section**. Do not split plots, metrics,
and discussion into separate top-level sections — that forces the reader to
jump back and forth to connect a number to its figure to its meaning.
...
> <one-sentence hypothesis claim, quoted from research/hypotheses/<slug>.md>

**Result:** <CONFIRMED / REJECTED / INCONCLUSIVE> — <one-sentence rationale citing the headline number>

| Success Criterion | Threshold | Actual | |
|---|---|---|---|
| <criterion> | <threshold> | <actual> | ✅ / ❌ |

---

## Finding 1 — <one-sentence claim as the heading>

<1–3 sentences stating the finding with specific numbers (mean, CI/std, n).>

![Figure 1: <short title>](../results/<slug>/<timestamp>/plots/<plot_file>.png)

*Figure 1. <Self-contained caption: what is plotted on each axis, what the
reader should look at, sample size, error bar / CI definition. The figure
should be understandable without the body text above or below it.>*

**Supporting data**

| | This run | Baseline | Δ |
|---|---|---|---|
| <metric cited in the prose> | <value ± std> | <baseline_value> | <±change> |

<1–3 sentences of interpretation: why does this finding matter, what in the
figure or table supports the claim, what caveats apply (range of validity,
sample size, assumptions). End with the link to the next finding or the
broader implication.>

## Finding 2 — <claim>

<Same structure: claim → figure + caption → supporting data → interpretation.>

<Repeat for each finding. Aim for 2–4 findings — fewer means the analysis is
shallow, more means the report is fragmented and should be split.>
...
<1–3 follow-up experiments or analyses>
```

**Authoring rules:**

1. **One figure per Finding.** If a finding needs two plots, combine them into a
   single multi-panel figure, or split the finding into two.
2. **Figure captions are self-contained.** State what is plotted, what to look
   at, sample size, and error-bar / CI definition. A reader should grasp the
   figure without scrolling.
3. **`Supporting data` tables hold only the numbers cited in the finding's
   prose.** Don't dump the full `metrics.json` — link to it via the `Results`
   path at the top instead.
4. **Do not add separate `Key Findings`, `Metrics`, `Plots`, or
   `Success Criteria` sections.** Findings are the body. The `Summary` table at
   the top is the only place pass/fail is pre-stated.
5. **Use `##` for each Finding** so editors and viewers can navigate the report
   via outline / TOC.
...
```bash
~/.claude/skills/research-stack/bin/rstack-learnings-log '{"skill":"report","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
